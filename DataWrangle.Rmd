...
ggplot(transformers, mapping = aes(x = congruency, y = flanker)) + 
  geom_bar(stat = "identity")+
  theme_classic(base_size = 12)+
  ylab("Mean Flanker Effect")
   geom_errorbar(aes(ymin= flanker - flankerSEM, ymax=flanker + flankerSEM), 
...
onlyincongruent <- RTS %>%
  filter(congruency == "I") %>%
  group_by(previous) %>%
  summarise(ImeanerRT = mean(Overreaction))

onlycongruentSEM <- c(std(onlycongruent$meanerRT))
onlyincongruentSEM <- c(std(onlyincongruent$ImeanerRT))
...
```{r}
tab <- as.table(onlycongruent, onlyincongruent)
```
