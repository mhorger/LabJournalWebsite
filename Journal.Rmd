...
if ((colSums(game) == 3||0) || (rowSums(game) == 3||0) || (diag(game) == 1||0)){
  win<-win+1
  }
...
game <- function(x){
     board<- as.matrix (
       (sample(0:1, 9, replace = TRUE)), nrows = 3, ncol = 3, byrow = FALSE, dimnames = NULL)
print(board)
}

     as.matrix(board, nrows = 3, ncol = 3)
     print(board)
}
    scores<-c(matrix(board, nrow=3, ncol = 3, byrow = FALSE, dimnames = NULL))
    print(scores)
}
...
board<-c(sample(0:1, 9, replace = TRUE))
