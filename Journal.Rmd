...
```{r}
write.txt(Captain, paragraph)

write.table(keanu, file = "keanu.txt", sep = "\t",
...
```{r}
keanu<-matrix(sample(1:20), ncol=20)
view (keanu)



fill.matrix = function(expr, nrow=20, ncol=20) {
    matrix(eval(expr, envir=list(x=nrow*ncol)), nrow=nrow, ncol=ncol)
}

fill.matrix(rexp(x, rate=1), nrow=20, ncol=20)
```
